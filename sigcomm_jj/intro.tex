...
Second, although one can save bandwidth by streaming only the area facing the viewer (around \fillme\% of whole frames), as commonly used in existing solutions(\eg~\cite{??,??,??,??}), it forces the encoder to use shorter video chunks (typically, one second) to keep up with the viewpoint movement, which significantly increases the bandwidth consumption.
...
However, we show that the perceived quality of a \vr video can be heavily influenced by several other factors---the {\em velocity} of the head movement, the {\em relative depth} of the viewed objects, and the {\em relative brightness} of the objects compared to those the user have just watched. 
